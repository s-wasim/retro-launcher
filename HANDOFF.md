...
| 12 | §1 "Size budget", as amended by row 2 above — a rule about the **APK** | 2.1.2 adds a second number to report: the launcher now writes generated icons to `cacheDir` under a 24-hour TTL and a 4 MB ceiling, so **installed** size grows by roughly 100 KB for a few hundred apps while package size is unchanged. The APK number stays the one the size rule governs; the cache number is reported, bounded and reclaimable. | The three-stage icon pipeline exists so that no icon ever ships in the package — that is why there are no image assets to cut. The cost of that choice was paying stage 2 in full on every cold start and again after every `LruCache` eviction: a `PackageManager` drawable load, an adaptive-icon crop and 576 nearest-ramp lookups, per app. Caching to disk moves that from once a launch to once a day, and the bytes it spends are in `cacheDir`, which the system may reclaim at any time and which the TTL and the ceiling bound regardless. Trading installed size for startup cost is the same trade row 2 already made for features, in the direction row 2 already chose. |
