...
| 30 | **How an icon gets to the screen** | Three stages — mark, converted real icon, letter tile — each rendered on demand, the result cached in memory as a bitmap already upscaled to the caller's pixel size. | Same three stages, unchanged in appearance. What changed is the cache: keyed without the pixel size (`core/IconCacheKey`, tested), storing each stage at its own source resolution — 16x16 mark, 24x24 converted — and upscaled nearest-neighbour by a filterless `BitmapDrawable` at draw time instead of eagerly. Stages 2 and 3 also persist as PNGs under `cacheDir` with a 24-hour TTL and a 4 MB ceiling (`icons/DiskIconCache`); stage 1 does not, being cheaper to redraw than to decode. `onTrimMemory` drops the memory tier and keeps the files. 2.1.2. | Reported: slow, and killed in the background. Both came from the same line. At `SIZE_ICON_CQW` on a 1080p screen an upscaled icon is 130x130 ARGB_8888 — 67 KB — and the size was part of the key, so the drawer, the dock and the sheet each stored their own copy of every app. A 4 MB budget therefore held about sixty of them on a device with several hundred apps: scrolling the drawer evicted continuously and re-ran stage 2's Binder round-trip and quantize for rows that had been on screen seconds earlier, which is the jank, and the launcher sat near its budget the whole time, which is what made it the process to kill. Caching the source resolution instead costs 2.3 KB per icon, fits a whole device in a fraction of the budget, and yields identical pixels for stage 2 — `createScaledBitmap(…, false)` is itself a filterless matrix scale, so this changes when it runs, not what it computes. Stage 1 can move a cell boundary by one device pixel at sizes that are not a multiple of 16, which is the one visible difference and is recorded in the code. Stage 3 is antialiased text and cannot be rebuilt from a smaller copy, so it alone keeps the size in its key; it is also the rarest, firing only for an app with no icon at all. |
| 31 | **What the launcher costs to leave running** | A dedicated render thread redrawing the sky at a fixed ~30fps for as long as the launcher is foreground; `AppRepository.load()` re-enumerating every installed app on each `onResume`, each search-overlay open and each dock-picker build; a minute ticker on a flat 60-second delay. | Frame pacing derived from the scene by `core/FrameBudget` (tested): ~24fps with rain, snow or lightning, 4fps for everything else that moves, capped at 1fps while a settled panel covers the sky, and stopped outright on `ACTION_SCREEN_OFF` and on focus loss. The enumeration is cached until a package broadcast or a category edit invalidates it. The ticker aligns to the wall-clock minute. `InstrumentedIconSource` is no longer wired. 2.1.3. | Reported: the launcher uses a lot of battery and has to stay resident. The sky was essentially the whole bill — a 108x230 buffer, several full-buffer passes, thirty CPU wake-ups a second, indefinitely — and almost none of it produced a different frame. The layer speeds say why: rain falls at 124–238 px/s and needs frames, but a cloud drifts at 0.3–0.9 px/s and crosses a pixel about every two seconds, so 4fps and 30fps emit the identical sequence of frames for it. The star twinkle and the sun's ray pulse are the binding constraint on the slow rate at ~15 samples a cycle, comfortably above where either steps. An earlier cut had a middle tier for clouds on the assumption that visible drift needed more than 4fps; the numbers retired it, and `FrameBudget`'s gates are now copies of `SkyRenderer`'s own so a layer that draws nothing cannot hold the rate up and one that draws something cannot be starved. Two second-order fixes came with it. The longer sleeps broke `stopThread`'s `join(500)` — it would give up while the thread slept on, null the reference, and let the next `resume()` start a **second** render thread with the first still about to call `lockCanvas` — so the sleep is now a monitor wait that stopping can cut short. And the unconditional `SCREEN_OFF` pause needed a guaranteed counterpart, since an activity a ROM never paused is never resumed either; regaining window focus is that counterpart, and `resume()` is idempotent. |
| 32 | **A second time zone on the clock** | Not in the prototype — one clock, the device's own. | A smaller line under the local time carrying one saved zone: city, time, and a `+1`/`-1` when the date there is not today's. Tap it to cycle the saved shortlist, long-press to open `ui/TimeZonePicker`; Settings → CLOCK & DATE gains a `SECOND TIME ZONE` toggle and a `ZONES` row onto the same picker. Up to six zones are stored, newline-joined like the dock. `core/ZoneList`, `core/ZoneLabel` and `core/ClockText` carry the logic, all tested. Off by default, `GONE` until a zone is saved. 2.2.1. | Requested as "the user picks the other zone from the default clock app". That is not buildable: Google's and Samsung's DeskClock keep their world-clock cities in their own private `SharedPreferences`, no content provider exposes them, and nothing short of `QUERY_ALL_PACKAGES` — which §0 row 10 forbids — would even make the packages visible. So the launcher keeps its own shortlist, which costs no permission and no asset, `TimeZone.getAvailableIDs()` being on the device already. Five details are load-bearing. The second line is built from the *same instant* as the local one rather than a fresh `currentTimeMillis()`, or the two could disagree about the minute and put a bogus `+1` on screen for one tick a day. `getTimeZone` answers an unknown id with GMT rather than failing, so an id the device does not know is checked against the table and the line hidden — a list carried from another device would otherwise show a confident wrong time. The day marker compares years before days, because 31 December against 1 January is where subtracting day-of-year gives 364 instead of -1. The picker stores an *index* and repairs it on every read, since removing a zone can leave it pointing past the end or, worse, silently at a different city. And `ClockText` exists at all because `renderTime` lived inside a `FrameLayout` and was therefore untestable — at one caller that was a tolerable risk, at two an off-by-one in the 12-hour wrap becomes two wrong clocks. The label is the id's last segment rather than `TimeZone.getDisplayName()`, which returns either a localised long name too wide for the line or a `GMT+05:00` that says nothing about where it is. |
| 33 | **Thunderstorm strength** | A `boolean`. WMO codes 95, 96 and 99 all mapped to one identical `Condition(0.97f, false, true)`, and `renderLightning` struck at a flat 0.12 chance per frame with the flash pinned to 1.0 — a distant rumble and a hailstorm rendered identically. | A 0-5 level (`core/ThunderIntensity`, tested) carried on `Weather.thunderLevel`, exposed to the renderer as 0..1 via `thunderScalar()`. Graded from two free, global sources already on the one GET the app makes: `cape` (Convective Available Potential Energy, J/kg), newly appended to the `current=` list, and the WMO code's own hail gradation. Strike chance now runs 0.04-0.30 per frame and flash brightness 0.35-1.0 across the scale. Persisted as `wxThunderLvl` beside the boolean. Not surfaced in the UI. 2.3.1. | Asked for: a quantitative reading of lightning intensity from the free API, with a realistic number even when the source is only text. Both exist and both were being discarded. The audit's other two candidates were rejected on evidence: `lightning_potential` (LPI, J/kg) is the most direct measure there is and a live call confirms it works, but only on the `dwd-icon` endpoint and only over Central Europe and North America — a second request for a number most of the world gets as null; `lifted_index` is free and global but adds nothing CAPE does not already say. The load-bearing rule is the negative one: **CAPE grades a storm and may never declare one.** It is potential energy, not an observation — a hot humid afternoon reads 3000+ J/kg under a cloudless sky — so the level is 0 unless the WMO code reports a thunderstorm, whatever CAPE says, and that false positive is the first thing both test suites pin. Where both are present the answer is the higher of the two, because hail is an observation and CAPE is a forecast quantity: a storm dropping heavy hail is extreme however the model reads. Code 95 floors at LIGHT rather than MODERATE since it covers "slight *or* moderate" and understating an unmeasurable storm is the safer error. Both render curves are floored well above zero — the lowest level is still a thunderstorm, and one that almost never flashes reads as a broken wallpaper rather than as weather. |
| 34 | **Where the moon's rise and set come from** | N/A — the prototype has one page and no cache, so the moon window is recomputed with everything else on every frame. | Computed by `core/LunarMath` from the coarse fix, and — 2.3.2 — computed there on *every* path that stores a day's times, not just the local-computation one. `SolarTimes` gained `hasMoonTimes()`/`withMoonTimes()` (tested) so "this day has no moon window" is a question the data layer asks rather than a NaN that travels. A cached day that has no window is repaired and written back. 2.3.2. | Reported: the moon has completely disappeared. It had, and permanently — the launcher was erasing its own moon roughly every half hour. The day's sunrise/sunset/moonrise/moonset live in one `SharedPreferences` entry keyed by date alone, written from two places. `solarTimes()` computed all four locally and stored them; the weather fetch stored what came back from the network. But Open-Meteo's daily block carries sunrise and sunset and **no moon at all**, so `WeatherParser.parseSolarTimes` could only build `SolarTimes`' sun-only form, whose moonrise and moonset are `Float.NaN` — and the fetch persisted that over the computed pair. From then on the date key matched, so `solarTimes()` returned the cached entry unread and never recomputed, and each fetch renewed the NaNs. `BodyPath.moonT` answers NaN for a window with a NaN end, `SkyRenderer` reads NaN as "the moon is not up", and nothing was drawn — at any hour, on any phase, on every device with location granted. The one load-bearing rule is the one the bug broke: **the moon window has exactly one source, and nothing may persist a day's times without passing through it.** `LunarMath` is pure local arithmetic — two dozen altitude samples, no I/O — so the fetch path computing it costs nothing worth weighing against a wallpaper with no moon in it. The repair on read exists because the bad entry outlives the upgrade: without it a user who installed the fix at noon would see no moon until midnight. Half-known windows are deliberately *not* fixed here and remain absent days: `LunarMath` reports only crossings inside the local calendar day, so on the ~2 days a month when the moon sets after midnight and does not set again, one end is NaN and the disc stays away. That is a separate and much smaller bug — a day here and there, not every day — and it wants the window to span midnight properly rather than a patch. **Done in 2.3.3 — see delta 35, which also measured it: 25 days a year, not "a day here and there".** |
| 35 | **A moon window that crosses midnight** | 2.3.2 gave the window one source but still built it from `LunarMath`'s scan of the local calendar day alone, so a window with one end on a neighbouring day came back half-known — and half-known draws exactly as much moon as none: `BodyPath.moonT` needs both ends. Delta 34 deferred it as "a day here and there". | `LunarMath.moonTimes` searches the adjoining day for whichever end today lacks, reports it as that day's own `[0, 24)` hour, and lets `moonT`'s existing wrap do the rest. Neighbours are scanned 26 hours rather than 24, and resolved through `LocalDate` rather than by adding 86,400,000 ms. A day with no crossing at all is now decided by the midday altitude instead of always reading as absent, and an end still missing after all that is anchored to the day's own edge. The invariant is explicit: a window is complete or it is absent, never partial. 2.3.3. | Measured rather than estimated: at Berlin, Karachi and New York alike, **25 days of 2026 drew no moon — every one of them a half-known window, none of them a genuine absence** — and after the change, zero. Delta 34's "a day here and there" was an undercount by an order of magnitude; it is a moonless night every other week. `moonT` already handled the wrap (a set earlier than the rise reads as "sets tomorrow"), so nothing in the renderer changed — the missing piece was only ever finding the crossing. Two cases surfaced only because the fix was tested across a full year at three longitudes rather than on a sample date, and both would have shipped as "mostly fixed": a 24-hour scan stops an hour short of a 25-hour daylight-saving fall-back day, which is precisely where 2026-11-02's missing New York moonrise sat; and above about 61° the window can exceed a day and a half, so the immediate neighbour does not contain the missing end either. The first wanted a longer scan, the second an honest fallback — if the moon is up at midnight it rose before today, and if it is still up at the end of the day it sets after, both true statements about today and both drawable. The remaining compromise is recorded here rather than left implicit: two hour-of-day floats cannot express a span longer than 24 hours, so a genuinely multi-day polar window is drawn as a full day rather than spread across several. That is a representation limit of `SolarTimes`, not a search bug, and it renders a moon rather than hiding one. |
| 36 | **The moon in a daylit sky** | The moon was faded out entirely once the sun cleared the horizon: `SkyRenderer.moonVisibility`'s sun term reached zero at `sunAlt = 0.10`, about half an hour after sunrise. Every daytime moon was erased. | The daylight term is now a *floor* rather than a cutoff, and the floor is set by the moon's lit fraction: `0.06 + 0.58·lit^1.5`, ramped in over `sunAlt` −0.05→0.35. The unlit limb fades on the daylight ramp separately, since by day it is the sky behind it and not a grey disc. 2.3.4. | Reported from Pakistan: "the moon is nowhere to be seen." It was not, for two-thirds of the day. A moon is above the horizon in daylight for roughly half of every lunar month — the ten-odd days around each quarter are exactly the ones where it hangs in an afternoon or morning sky — and the wallpaper drew none of it. On 2026-09-19 in Islamabad the moon rose at 13:47 and nothing appeared until about 18:00. What daylight actually costs the moon is contrast, and contrast scales with how much of the disc is lit, so the fade is driven by the lit fraction instead of being flat. The pairing is self-limiting with no special case: a full moon is by definition opposite the sun, so it is never up while the sun is high enough for the floor to bite. Measured on the 108×234 buffer at 15:00: a 84%-lit gibbous peaks 39 luma above the sky across ~415 pixels, a quarter 20 across ~208, a 16% crescent 3–6 across ~24 — visible, subtle and all but gone, which is the order the real sky puts them in. |
| 37 | **One moon window per calendar day was the wrong unit** | 2.3.3 found both ends of a window that crosses midnight, but still built *one* pair per calendar day out of the day's first rise and the day's first set. On the roughly half of all days that hold the tail of one up-period in the small hours and the start of the next later on, those two crossings belong to different windows. | `LunarMath.moonWindow(lat, lon, instant, zone)` replaces `moonTimes(..., date, ...)`: it returns the up-period *containing that instant*, or the next one to begin after it. Hours are offsets from local midnight and may be negative or past 24, so `BodyPath.moonT` is a plain lerp and the wrapped encoding is gone. The window is no longer persisted — `SolarTimes`' moon half is recomputed on the minute tick, and `Prefs` lost `K_SOL_MOONRISE`/`K_SOL_MOONSET`. 2.3.4. | Found while auditing delta 36. A calendar day is not a lunar day, and no single rise/set pair describes both up-periods a day can hold — so the pair that was built described neither. On 2026-09-21 in Pakistan the moon set at 00:19 (the tail of the 20th's rise) and rose again at 15:12 to set at 01:19 on the 22nd; the code paired 15:10 with 00:20 and retired the evening moon an hour early. The mirror case ran the other way: on 2026-09-05 it drew a moon from 23:00 that did not rise until 00:05. Measured against an independent higher-precision ephemeris over 2026, sampled every ten minutes at five locations, the worst single day's disagreement falls from 70–170 minutes to 20, and moon-drawn-when-it-is-not-there drops by about a third. The two polar checks improve on the same measure without reaching that accuracy — 69°N from 170 minutes to 90, 78°N from 350 to 220 — because there the moon skims the horizon for hours and the error is the low-precision position formula's, not the pairing's; fixing that would mean a better ephemeris, not a better scan. Asking about an instant removes the ambiguity rather than encoding around it, and since the answer is instant-specific there is nothing left to cache — which also retires delta 34's failure mode at the root, because a window that is never stored can never be stored wrong. |
...
On top of `HANDOFF.md` §1's constraints:
