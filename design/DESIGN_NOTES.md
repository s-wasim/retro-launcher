...
| 17 | **Panel compositing** | N/A — a DOM prototype has no surface-compositing layer to desync. | `SkyView` is a `TextureView`, not a `SurfaceView`. Added 2026-08-31. | A `SurfaceView`'s pixels are composited by SurfaceFlinger as an independent hole-punch layer, outside the RenderThread pipeline that animates the panels above it. Under a slow drag the two compositors could fall a frame out of lockstep, briefly showing a stale, faded copy of a panel's own last frame behind the live one — the "ghost panel" bug. A `TextureView` composites inside the same pipeline as everything else, so no second compositor exists to desync. See `docs/superpowers/specs/2026-08-31-panel-fixes-and-launcher-controls-design.md` §1. |
...
| 19 | **Lock the device** | None. | Long-press on Home calls `DevicePolicyManager#lockNow()`, gated behind a one-time Device Admin activation dialog (`LockAdminReceiver`, no policies requested beyond what any active admin already has). Status surfaced in Settings → PERMISSIONS. Added 2026-08-31. | Requested. |
| 20 | **Default launcher** | None. | A dismiss-when-satisfied banner on Home plus a durable status row in Settings → PERMISSIONS, both firing the system role-request flow (`RoleManager.ROLE_HOME` on API 29+, `Settings.ACTION_HOME_SETTINGS` below that). Added 2026-08-31. | Requested: the app should be able to ask to become the default launcher without a manual trip through system settings first. |
| 21 | **Notification shade** | None. | A swipe down starting inside the top system-bar inset on Home expands the shade via reflection on `StatusBarManager#expandNotificationsPanel()` (`EXPAND_STATUS_BAR`, normal protection), a silent no-op if unavailable. Added 2026-08-31. | Requested. Otherwise-inert real estate — a swipe down elsewhere on Home already does nothing (§9 delta 12's inset carve-out, applied to the top edge). |
